...
function animate(mat, isDepth) {
  ...
  mat.customProgramCacheKey = () => 'creature3' + (isDepth ? 'D' : 'C');
  return mat;
}
...
R.buildCreatures = function (world, sim, Q, CAP, MT_KEYS) {
  ...
  let shadowsOn = Q !== 'low';
  const mk = (k, detail, cap) => { DET = detail; const geo = R.merge(MODELS[k]().filter(p => detail || !p.fine), { extra: true }); DET = 1;
